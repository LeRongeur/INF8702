...
CIntersection CQuadrique::Intersection(const CRayon& Rayon)
{
	...
	REAL t = RENDRE_REEL(0.0);

	REAL Aq = A*xd*xd + B*yd*yd + C*zd*zd + D*xd*yd + E*xd*zd + F*yd*zd;
	REAL Bq = RENDRE_REEL(2.0) * A*xo*xd + RENDRE_REEL(2.0) * B*yo*yd + RENDRE_REEL(2.0) * C*zo*zd + D*(xo*yd + yo*xd) + E*xo*zd + F*(yo*zd + yd*zo) + G*xd + H*yd + J*zd;
	...
	if (Aq == 0) {
		...
		if (delta >= 0) {
			t = (-Bq - RENDRE_REEL(sqrt(delta))) / (RENDRE_REEL(2.0) * Aq);
			if (t < 0) {
				t = (-Bq + sqrt(delta)) / (RENDRE_REEL(2.0) * Aq);
				if (t > 0) {
					intersection = true;
				}
			} else {
	...
	if (intersection) {
		...
		CVecteur3 normale = CVecteur3::Normaliser(CVecteur3(RENDRE_REEL(2.0) * A*pos.x + D*pos.y + E*pos.z + G,
			RENDRE_REEL(2.0) * B*pos.y + D*pos.x + F*pos.z + H,
			RENDRE_REEL(2.0) * C*pos.z + E*pos.x + F*pos.y + J));
